...
type Blackboard struct { //nolint:govet
	Threats []ThreatFact
...
func SelectGoal(bb *Blackboard, profile *SoldierProfile, isLeader, hasPath bool) GoalKind {
	goals := []GoalKind{
		GoalAdvance,
		GoalMaintainFormation,
		GoalRegroup,
	...
	best := goals[0]
	bestVal := goalUtilSingle(bb, profile, isLeader, hasPath, best)
	for i := 1; i < len(goals); i++ {
		g := goals[i]
		u := goalUtilSingle(bb, profile, isLeader, hasPath, g)
		if u > bestVal {
			best = g
			bestVal = u
		}
...
func goalUtilSingle(bb *Blackboard, profile *SoldierProfile, isLeader, hasPath bool, goal GoalKind) float64 { //nolint:gocognit,gocyclo
	bb.ensureInternalDefaults()
